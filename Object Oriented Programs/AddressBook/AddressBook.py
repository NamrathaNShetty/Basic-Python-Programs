...
class AddressBook:
    ...
    """
        ...
        if len(self.list) >= 1:
            ...
            try:
                for i in range(len(self.list)):
                    logging.info(self.list[i]['id'], "\t\t\t\t", self.list[i]['name'],  "\t\t\t\t",
                        self.list[i]['mobile'], "\t\t\t\t", self.list[i]['address'], "\t\t\t\t",
                        self.list[i]['zip'], "\t\t\t\t", self.list[i]['city'], "\t\t\t\t",
                        self.list[i]['state'], "\n")
            except ValueError:
...
if __name__ ==  '__main__':
    try:
        while True:
            ...
            op = int(input("Press \n 1. Add New User 2. To Existing User 3. To Quit \n"))
            if op == 1:
